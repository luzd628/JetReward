...
private fun BottomBar(
    ...
    BottomNavigation(
        ...
        //untuk membedakan button yg sedang aktif dan tidak
        val navBackStackEntry by navController.currentBackStackEntryAsState()
        val currentRoute = navBackStackEntry?.destination?.route
        ...
        BottomNavigation {
            navigationItem.map { item ->
                BottomNavigationItem(
                    ...
                    onClick = {
                        //fungsi utk navigasi ke route sesuai item yg dipilih:
                        navController.navigate(item.screen.route){
                            //fungsi untuk kembali langsung ke halaman awal:
                            popUpTo(navController.graph.findStartDestination().id){
                                saveState = true
                            }
                            //mengembalikan state ketika item dipilih lagi save&restore:
                            restoreState = true
                            //digunakan agar tidak ada halaman yang double ketika memilih item ulang yg sama:
                            launchSingleTop = true
                        }
                    }
                )
